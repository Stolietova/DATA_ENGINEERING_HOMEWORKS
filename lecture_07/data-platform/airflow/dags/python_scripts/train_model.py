...
def process_iris_data(**kwargs):
    ...
    try:
        ...
        results_df = pd.DataFrame({
            ...
            'data_date': [execution_date_str, execution_date_str],
        ...
        feature_importance_df['data_date'] = execution_date_str

        # Save model as file
        model_filename = f"iris_model_{execution_date_str}.pkl"
        full_model_path = MODEL_PATH_BASE + model_filename

        if not os.path.exists(MODEL_PATH_BASE):
            os.makedirs(MODEL_PATH_BASE, exist_ok=True) 
            print(f"Directory created for models: {MODEL_PATH_BASE}")
       
        joblib.dump(clf_top5, full_model_path)
        print(f"Model saved successfully to: {full_model_path}")
        kwargs['ti'].xcom_push(key='model_path', value=full_model_path)

        ti.xcom_push(key='top_5_features', value=top_features)
        ...
        with engine.connect() as connection:
            ...
            delete_metrics_query = f"""
                DELETE FROM ml_results.iris_model_metrics
                WHERE data_date = '{execution_date_str}';
                """
            connection.execute(text(delete_metrics_query))
                
            delete_importance_query = f"""
                DELETE FROM ml_results.iris_feature_importance
                WHERE data_date = '{execution_date_str}';
                """
            connection.execute(text(delete_importance_query))
        ...
        error_message = (
            f"TECHNICAL FAILURE for {execution_date_str}. Reason: {type(e).__name__}: {str(e)}"
        )
